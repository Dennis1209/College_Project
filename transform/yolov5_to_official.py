...
for i in range(1001,2001) : #圖片編號
    ...
    with open(in_filename,"r") as in_f: 
        for line in in_f: 
            l1 = line.replace('\n','')
            l1 = line.split(' ')
            num1 = math.floor((float(l1[1]) - float(l1[3])/2) * 1920)
            num2 = math.floor((float(l1[2]) - float(l1[4])/2) * 1080)
            num3 = math.floor(float(l1[3]) * 1920)
            num4 = math.floor(float(l1[4]) * 1080)
            num5 = num1 + num3
            num6 = num2 + num4
            if(num1 < 0):
                num1 = 0
